...
async function onSubmitEdit(event: FormSubmitEvent<EditSchema>) {
  ...
  try {
    const response = await $fetch<{ admin: AdminAccount }>(`${API_BASE}/api/admin/account`, {
      method: 'PATCH',
      headers: {
    ...
    const updatedAdmin = response.admin
    ...
    if (user.value && updatedAdmin) {
      ...
      if (updatedAdmin.gender !== undefined) user.value.gender = updatedAdmin.gender
    }
...
const isCreateAdminSubmitting = ref(false)

const createAdminState = reactive({
  email: '',
  password: '',
  firstName: '',
  lastName: '',
...
type CreateAdminSchema = typeof createAdminState

function validateCreateAdmin(state: Partial<CreateAdminSchema>): FormError[] {
  const errors: FormError[] = []
...
function openCreateAdminModal() {
  createAdminState.email = ''
  createAdminState.password = ''
  createAdminState.firstName = ''
  createAdminState.lastName = ''
  createAdminState.gender = 'MALE'
  isCreateAdminOpen.value = true
}

async function onSubmitCreateAdmin(event: FormSubmitEvent<CreateAdminSchema>) {
  if (isCreateAdminSubmitting.value) return
  isCreateAdminSubmitting.value = true
  try {
    await $fetch(`${API_BASE}/api/admin/register`, {
      ...
      body: {
        email: createAdminState.email.trim(),
        password: createAdminState.password,
        firstName: createAdminState.firstName.trim(),
        lastName: createAdminState.lastName.trim(),
        gender: createAdminState.gender,
      },
    })
    ...
    isCreateAdminSubmitting.value = false
  }
}
...
async function onSubmitChangePassword(event: FormSubmitEvent<ChangePasswordSchema>) {
  ...
  try {
    await $fetch(`${API_BASE}/api/admin/account/password`, {
      method: 'PATCH',
      headers: {
        Authorization: `${useAuthToken().value}`,
      },
...
async function onProfilePicSelected(event: Event) {
  ...
  try {
    ...
    const response = await $fetch<{ admin?: AdminAccount; profileImageURL?: string }>(`${API_BASE}/api/admin/profile-pic`, {
      method: 'POST',
      headers: {
        Authorization: `${useAuthToken().value}`,
      },
    ...
    const newUrl = response?.admin?.profileImageURL ?? response?.profileImageURL
    if (user.value && newUrl) {
      user.value.profileImageURL = newUrl
    } else if (user.value && response?.admin) {
      user.value.profileImageURL = (response.admin as AdminAccount).profileImageURL
    }
        ...
        <template #header>
          <div class="flex items-center justify-between w-full">
            ...
            <UButton
              ...
              @click="isCreateAdminOpen = false"
            />
          </div>
        </template>
        <template #body>
          <UForm :validate="validateCreateAdmin" :state="createAdminState" class="space-y-4" @submit="onSubmitCreateAdmin">
            <UFormField label="First Name" name="firstName" required block>
              <UInput v-model="createAdminState.firstName" class="w-full" placeholder="First name" />
            </UFormField>

            <UFormField label="Last Name" name="lastName" required block>
              <UInput v-model="createAdminState.lastName" class="w-full" placeholder="Last name" />
            </UFormField>

            <UFormField label="Email Address" name="email" required block>
              <UInput v-model="createAdminState.email" type="email" class="w-full" placeholder="Email" />
            </UFormField>

            <UFormField label="Password" name="password" required block>
              <UInput v-model="createAdminState.password" type="password" class="w-full" placeholder="Password" />
            </UFormField>

            <UFormField label="Gender" name="gender" required block>
              <USelect
                v-model="createAdminState.gender"
                :items="genderOptions"
                placeholder="Select gender"
                class="w-full"
              />
            ...
            <div class="flex justify-end gap-2">
              <UButton type="button" variant="outline" :disabled="isCreateAdminSubmitting" @click="isCreateAdminOpen = false">
                Cancel
              </UButton>
              <UButton type="submit" :loading="isCreateAdminSubmitting" :disabled="isCreateAdminSubmitting">
                Create Account
              </UButton>
            </div>
          </UForm>
