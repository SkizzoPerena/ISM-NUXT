...
function uploadFileWithProgress(file: File): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const url = `${API_BASE}/api/admin/group-submission/${groupSubmissionId.value}`
    xhr.open('POST', url)
...
async function submitVideoLink() {
  ...
  try {
    if (file) {
      ...
      await $fetch(`${API_BASE}/api/admin/group-submission/${groupSubmissionId.value}/link`, {
        method: 'PATCH',
        headers: { Authorization: `${useAuthToken().value}` },
    ...
    await refreshNuxtData(`group-submission-${groupSubmissionId.value}`)
  } catch (e) {
...
async function markAsLiveSubmission(close: () => void) {
  ...
  try {
    await $fetch(`${API_BASE}/api/admin/group-submission/${groupSubmissionId.value}/live`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
    })
    await refreshNuxtData(`group-submission-${groupSubmissionId.value}`)
...
async function submitRubricAnswers() {
  ...
  try {
    await $fetch(`${API_BASE}/api/admin/group-submission/${groupSubmissionId.value}`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
...
async function submitReject() {
  ...
  try {
    await $fetch(`${API_BASE}/api/admin/group-submission/${groupSubmissionId.value}/reject`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
...
async function generateAnalysis() {
  ...
  try {
    await $fetch(`${API_BASE}/api/admin/group-submission/${groupSubmissionId.value}/analysis`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
    })
    await refreshNuxtData(`group-submission-${groupSubmissionId.value}`)
...
async function submitOverrideAnalysis() {
  ...
  try {
    await $fetch(`${API_BASE}/api/admin/group-submission/${groupSubmissionId.value}/override`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
