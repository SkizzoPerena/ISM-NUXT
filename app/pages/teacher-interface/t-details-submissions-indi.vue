...
const { data, status } = await useAsyncData<AssessmentPageData | null>(
  `teacher-assessment-page-${individualSubmissionId.value}`,
  async () => {
    if (!individualSubmissionId.value) return null
    let assessmentId: string | undefined
    try {
      const submissionResponse: any = await $fetch(`${API_BASE}/api/teacher/individual-submission/${individualSubmissionId.value}`, {
