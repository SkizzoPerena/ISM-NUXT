...
const { data, status } = await useAsyncData('rubrics',
  () => $fetch('https://noteworthy-z9k0.onrender.com/api/teacher/rubrics', {
    headers: {
  ...
  }),
  {
    transform: (response: any): Rubric[] => {
      ...
      // The /api/teacher/rubrics endpoint returns a direct list of rubric templates.
...
const rubriccolumns: TableColumn<Rubric>[] = [
  {
    accessorKey: 'title',
    header: 'Title',
    cell: ({ row }) => h(NuxtLink, { to: `../teacher-interface/t-details-rubrics?id=${row.original._id}`,  class: 'text-primary font-medium hover:underline' }, { default: () => row.getValue('title') })
    },
    {
    ...
    <UPageCard>
        ...
        <div style="margin-left: auto">
          <UInput :model-value="table1?.tableApi?.getColumn('title')?.getFilterValue() as string" class="max-w-sm mr-5"
            ...
            <UButton label="Add New Rubric" to="../teacher-interface/t-create-rubrics"/>
