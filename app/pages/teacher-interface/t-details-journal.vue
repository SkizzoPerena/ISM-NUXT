...
const { data: journal, status, refresh: refreshJournal } = await useAsyncData<JournalDetail | null>(
  `teacher-journal-${journalId.value}`,
  async () => {
    const response: any = await $fetch(`${API_BASE}/api/teacher/journals/${journalId.value}`, {
      headers: { Authorization: `${useAuthToken().value}` },
    });
    console.log('Teacher Journal API Response:', response);
    return response?.journal || response?.data || response || null;
  },
  {
    watch: [journalId]
  }
...
async function onSubmitEdit() {
  ...
  try {
    ...
    await $fetch(`${API_BASE}/api/teacher/journals/${journalId.value}`, {
      method: 'PATCH',
      headers: {
...
async function onDeleteJournal() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/journals/${journalId.value}/archive`, {
      method: 'DELETE',
      headers: {
        Authorization: `${useAuthToken().value}`,
      },
