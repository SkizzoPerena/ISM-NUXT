...
const { data: section, status, refresh: refreshSection } = await useAsyncData<SectionDetail>(
  `teacher-section-${sectionId.value}`,
  () => $fetch(`${API_BASE}/api/teacher/sections/${sectionId.value}`, {
    headers: {
...
const { data: journalEntries, status: journalEntriesStatus, refresh: refreshJournalEntries } = await useAsyncData<Journal[]>(
  `teacher-section-journals-${sectionId.value}`,
  () => $fetch(`${API_BASE}/api/teacher/journal-sections/section/${sectionId.value}`, {
    headers: {
      Authorization: `${useAuthToken().value}`
    },
    onResponse({ response }) {
...
const { data: groupSubmissionsRaw, status: groupSubmissionsStatus } = await useAsyncData<GroupSubmission[]>(
  `teacher-section-group-submissions-${sectionId.value}`,
  () => $fetch(`${API_BASE}/api/teacher/group-submission/section/${sectionId.value}`, {
    headers: {
      Authorization: `${useAuthToken().value}`
    },
    onResponse({ response }) {
...
const columns: TableColumn<any>[] = [
  {
    ...
    cell: ({ row }) => h('div', { class: 'flex items-center gap-3' }, [
      ...
      h('div', undefined, [
        h(NuxtLink, { to: `/teacher-interface/t-profile-student?id=${row.original.id}`, class: 'text-primary font-medium hover:underline' }, { default: () => row.getValue('name') }),
...
async function onSubmitEdit(event: FormSubmitEvent<EditSchema>) {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/sections/${sectionId.value}`, {
      method: 'PATCH',
      headers: {
        Authorization: `${useAuthToken().value}`
      },
...
async function confirmDeleteSection() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/sections/archive/${sectionId.value}`, {
      method: 'DELETE',
      headers: {
        Authorization: `${useAuthToken().value}`
      }
    ...
    await navigateTo('/teacher-interface/t-sections')
  } catch (error) {
...
async function confirmAddStudent() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/sections/${sectionId.value}/assign-student/${addStudentSelectedId.value}`, {
      method: 'POST',
      headers: { Authorization: `${useAuthToken().value}` }
    })
...
async function confirmUnassignStudent() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/sections/${sectionId.value}/unassign-student/${studentToUnassign.value.id}`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` }
    })
...
async function confirmAssignAssessment() {
  ...
  const url = outputType === 'individual'
    ? `${API_BASE}/api/teacher/assessment-sections/assign/individual`
    : `${API_BASE}/api/teacher/assessment-sections/assign/group`
  try {
    await $fetch(url, {
      method: 'POST',
      headers: { Authorization: `${useAuthToken().value}` },
...
async function confirmUnassignAssessment() {
  ...
  const url = type === 'individual'
    ? `${API_BASE}/api/teacher/assessment-sections/unassign/individual/${assignmentId}`
    : `${API_BASE}/api/teacher/assessment-sections/unassign/group/${assignmentId}`
  try {
    await $fetch(url, {
      method: 'DELETE',
      headers: { Authorization: `${useAuthToken().value}` },
...
async function confirmAssignJournal() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/journal-sections/assign`, {
      method: 'POST',
      headers: { Authorization: `${useAuthToken().value}` },
      body: {
...
async function confirmUnassignJournal() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/journal-sections/${assignmentId}`, {
      method: 'DELETE',
      headers: { Authorization: `${useAuthToken().value}` },
    })
...
const groupSubmissionColumns: TableColumn<GroupSubmission>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return h(NuxtLink, { to: `/teacher-interface/t-details-submissions-group?id=${submissionId}`, class: 'text-primary font-medium hover:underline' }, { default: () => title })
    }
  },
  {
...
const journalcolumns: TableColumn<Journal>[] = [
  {
    ...
    header: 'Title',
    meta: { class: { td: 'w-1/2' } },
    cell: ({ row }) => h(NuxtLink, { to: `/teacher-interface/t-details-journal?id=${row.original._id}`, class: 'text-primary font-medium hover:underline' }, { default: () => row.getValue('title') })
  },
  {
    accessorKey: 'endDate',
    header: 'Date',
