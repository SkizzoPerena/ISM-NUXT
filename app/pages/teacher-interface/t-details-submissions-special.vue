...
const { data: submission, status: submissionStatus } = await useAsyncData<SpecialSubmission | null>(
  ...
  async () => {
    if (!specialSubmissionId.value) return null
    try {
      const response: any = await $fetch(`${API_BASE}/api/teacher/special-submission/${specialSubmissionId.value}`, {
        headers: { Authorization: `${useAuthToken().value}` },
      })
...
function uploadFileWithProgress(file: File): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const url = `${API_BASE}/api/teacher/special-submission/${specialSubmissionId.value}`
    xhr.open('POST', url)
...
async function submitVideoLink() {
  ...
  try {
    if (file) {
      ...
      await $fetch(`${API_BASE}/api/teacher/special-submission/${specialSubmissionId.value}/link`, {
        method: 'PATCH',
        headers: { Authorization: `${useAuthToken().value}` },
    ...
    await refreshNuxtData(`teacher-special-submission-${specialSubmissionId.value}`)
  } catch (e) {
...
async function markAsLiveSubmission(close: () => void) {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/special-submission/${specialSubmissionId.value}/live`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
    })
    await refreshNuxtData(`teacher-special-submission-${specialSubmissionId.value}`)
...
async function submitRubricAnswers() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/special-submission/${specialSubmissionId.value}`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
...
async function submitReject() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/special-submission/${specialSubmissionId.value}/reject`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
...
async function generateAnalysis() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/special-submission/${specialSubmissionId.value}/analysis`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
    })
    await refreshNuxtData(`teacher-special-submission-${specialSubmissionId.value}`)
...
async function submitOverrideAnalysis() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/special-submission/${specialSubmissionId.value}/override`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
