...
const { data: submission, status: submissionStatus } = await useAsyncData<GroupSubmission | null>(
  ...
  async () => {
    if (!groupSubmissionId.value) return null
    try {
      const response: any = await $fetch(`${API_BASE}/api/teacher/group-submission/${groupSubmissionId.value}`, {
        headers: { Authorization: `${useAuthToken().value}` }
      })
...
function uploadFileWithProgress(file: File): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    formData.append('groupSubmission', file)
    const xhr = new XMLHttpRequest()
    const url = `${API_BASE}/api/teacher/group-submission/${groupSubmissionId.value}`
    xhr.open('POST', url)
...
async function submitVideoLink() {
  if (!groupSubmissionId.value) return
  ...
  try {
    if (file) {
      ...
      await $fetch(`${API_BASE}/api/teacher/group-submission/${groupSubmissionId.value}/link`, {
        method: 'PATCH',
        headers: { Authorization: `${useAuthToken().value}` },
    ...
    await refreshNuxtData(`teacher-group-submission-${groupSubmissionId.value}`)
  } catch (e) {
...
async function markAsLiveSubmission(close: () => void) {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/group-submission/${groupSubmissionId.value}/live`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
    })
    await refreshNuxtData(`teacher-group-submission-${groupSubmissionId.value}`)
...
async function submitRubricAnswers() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/group-submission/${groupSubmissionId.value}`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
...
async function submitReject() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/group-submission/${groupSubmissionId.value}/reject`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
...
async function generateAnalysis() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/group-submission/${groupSubmissionId.value}/analysis`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
    })
    await refreshNuxtData(`teacher-group-submission-${groupSubmissionId.value}`)
...
async function submitOverrideAnalysis() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/group-submission/${groupSubmissionId.value}/override`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
      ...
      <UPageCard class="mt-8">
        <UContainer>
          <UPageGrid>
            ...
            <!-- Rubric Q&A when already answered + Analysis section -->
            ...
            <!-- Rubric answers dialog (page-wide) -->
