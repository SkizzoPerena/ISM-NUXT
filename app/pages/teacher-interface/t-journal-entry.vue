...
onMounted(() => {
  if (!journalEntryId.value) {
    navigateTo('/teacher-interface/t-journals')
  }
})
...
const { data: journalEntryData, status, refresh: refreshJournalEntry } = await useAsyncData<JournalEntryResponse | null>(
  ...
  async () => {
    ...
    try {
      const response = await $fetch(`${API_BASE}/api/teacher/journal-entry/${journalEntryId.value}`, {
...
async function submitJournalAnswers() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/journal-entry/answers`, {
      method: 'PATCH',
      headers: {
...
async function submitRepertoireEntry() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/journal-entry/entries`, {
      method: 'PATCH',
      headers: {
        Authorization: `${useAuthToken().value}`,
        'Content-Type': 'application/json',
...
async function submitOverrideAnalysis() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/journal-entry/${journalEntryId.value}/override`, {
      method: 'PATCH',
      headers: {
        Authorization: `${useAuthToken().value}`,
        'Content-Type': 'application/json',
...
async function generateOrRegenerateAnalysis() {
  ...
  try {
    await $fetch(`${API_BASE}/api/teacher/journal-entry/${journalEntryId.value}/analysis`, {
      method: 'PATCH',
      headers: { Authorization: `${useAuthToken().value}` },
    })
